...
export default function StaffInbox({ getToken, role, focusClientId = null, focusClientName = null }) {
  ...
  // Track "was the staff member at the bottom" continuously (not just when messages
  // change), so the iOS keyboard effect below knows the answer as of right before
  // the viewport shrank rather than after — checking isNearBottom() after the
  // shrink would measure against the already-smaller viewport.
  ...
  // only when the staff member was already reading the latest message before it opened.
  useEffect(() => {
  ...
  async function scheduleMessage() {
    ...
    if (!schedDate || !schedTime) { alert('Please pick a date and time.'); return }
    ...
    if (Number.isNaN(sendAt.getTime())) { alert('That date and time is not valid.'); return }
    if (sendAt.getTime() <= Date.now()) { alert('Pick a date and time in the future.'); return }
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json().catch(() => ({}))
        alert(err.error ?? 'Could not schedule message')
      }
    } catch { alert('Could not schedule message') }
  ...
  async function cancelScheduled(scheduledId) {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}))
        alert(err.error ?? 'Could not cancel this scheduled message')
      }
      ...
      fetchScheduled()
    } catch { alert('Could not cancel this scheduled message') }
  }
