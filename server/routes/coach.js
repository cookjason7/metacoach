...
router.post('/chat', requireAuth(), chatLimit, async (req, res, next) => {
  try {
    ...
    const { rows: countRows } = await pool.query(
      `SELECT COUNT(*) AS total FROM coaching_conversations WHERE user_id = $1`,
      [dbUserId],
    )
    const conversationCount = parseInt(countRows[0]?.total ?? '0', 10)
    ...
    if (isAiClient && message && conversationCount === 1) {
      const icebreakerReply = `${message} — I love it. And I want you to know right now, it is not going anywhere. This is not a program that takes away the things you love. It is a program that builds a version of you who knows how to enjoy them and keep moving forward. That is a completely different game than anything you have tried before.\n\nHere is how we start. Your only job this week is to take a photo of everything you eat and drink. That is it. Do not change a single thing about your food — eat exactly how you normally eat. We just need to see your baseline first.\n\nHere is how to do it: tap the orange plus button at the bottom of the app, tap Food Photo, choose which meal it is, take a photo, and add a quick description of what you are eating. Something like: two cupcakes, or a slice of pizza. That is all I need.\n\nNo pressure. No perfection. Just photos. Can you do that this week?`
